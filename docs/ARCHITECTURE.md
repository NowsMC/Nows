...
Each supported adapter should expose the same stable mc adapter API names, for example `space.nows.mcnows.mc.api.MinecraftApi`, `RegistryApi` and `DataPacks`, while using that Minecraft version's real classes internally. This lets 26.2 use `Identifier` and `ResourceKey` setup while 1.20.1 uses `ResourceLocation`, without forcing mods to rewrite their Nows-facing calls.
