...
public final class ModListScreen extends Screen {
    ...
    private final class ModEntry extends ObjectSelectionList.Entry<ModEntry> {
        ...
        public void renderContent(
            ...
            graphics.drawString(font, trim(font, descriptor.name(), width - 40), x + 30, y, TEXT, true);
    ...
    private static final class ModDetailsPanel extends AbstractScrollArea {
        ...
        ModDetailsPanel(int x, int y, int width, int height) {
            super(x, y, width, height, Component.literal("Mod details"));
        }
        ...
        private void renderPanel(GuiGraphics graphics) {
            graphics.fill(getX(), getY(), getX() + getWidth(), getY() + getHeight(), PANEL);
            renderBorder(graphics, getX(), getY(), getWidth(), getHeight());
        }

        private void renderBorder(GuiGraphics graphics, int x, int y, int width, int height) {
    ...
    private sealed interface DetailLine permits TextLine, WrappedLine, SpacerLine {
        ...
        int render(GuiGraphics graphics, Font font, int x, int y, int width);
    }
    ...
    private record TextLine(String text, int color, boolean shadow, int extraBottom) implements DetailLine {
        ...
        @Override
        public int render(GuiGraphics graphics, Font font, int x, int y, int width) {
            graphics.drawString(font, trim(font, text, width), x, y, color, shadow);
    ...
    private static Identifier icon(ModDescriptor descriptor) {
        ...
        if (icon.startsWith("assets/")) {
            ...
            if (slash > 0 && slash + 1 < rest.length()) {
                return Identifier.fromNamespaceAndPath(rest.substring(0, slash), rest.substring(slash + 1));
            }
        }
