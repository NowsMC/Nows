...
public final class ModListScreen extends Screen {
    ...
    private static final class ModDetailsPanel extends AbstractScrollWidget {
        ...
        private void renderBackground(PoseStack graphics) {
            GuiComponent.fill(graphics, getX(), getY(), getX() + getWidth(), getY() + getHeight(), PANEL);
        ...
        @Override
        protected void renderContents(PoseStack graphics, int mouseX, int mouseY, float delta) {
            renderBackground(graphics);
            int x = getX() + 12;
            int y = getY() + 10;
    ...
    private record WrappedLine(String text, int color, int extraBottom) implements DetailLine {
        ...
        public int render(PoseStack graphics, Font font, int x, int y, int width) {
            for (var line : font.split(Component.literal(text), width)) {
                GuiComponent.drawString(graphics, font, line, x, y, color);
                y += font.lineHeight;
            }
