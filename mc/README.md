...
Keep Minecraft's loading/splash screen vanilla-owned. A version adapter may draw lightweight diagnostics on top of that splash, such as heap/offheap/CPU and the current Nows phase, but must not replace it with a Nows-owned panel or fake main progress screen. Keep the vanilla game loading bar visible, and render Nows progress as a secondary vanilla-style bar with useful loader facts such as the active phase, active mod entrypoint, Mixin config or resource pack. Put changing counters in fixed label/value positions so the whole line does not shift while `current/total` values update. If a version needs detailed loader progress, attach Nows work to the same reload/loading lifecycle that vanilla already uses so the native progress screen reports real work. This rule should hold for older adapters such as 1.12.2 too, even when the native screen is not called `LoadingOverlay`.
