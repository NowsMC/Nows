...
package space.nows.mcnows.mc.internal.nbt;
...
public enum NbtApiImpl implements NbtApi {
    ...
    public String getString(ListTag list, int index, String fallback) {
        return index >= 0 && index < list.size() ? list.getStringOr(index, fallback) : fallback;
    }

    @Override
    public int getInt(ListTag list, int index, int fallback) {
        return index >= 0 && index < list.size() ? list.getIntOr(index, fallback) : fallback;
    }

    @Override
    public CompoundTag getCompound(ListTag list, int index) {
        return index >= 0 && index < list.size() ? list.getCompoundOrEmpty(index) : compound();
    }
}
